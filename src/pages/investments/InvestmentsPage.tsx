...
export default function InvestmentsPage() {
  ...
  const totalActual = investments.reduce(
    (sum, inv) => sum + calcMetrics(inv).valorActual, 0
  );
  const totalVista = vistaInvs.reduce(
    (sum, inv) => sum + calcMetrics(inv).valorActual, 0
  );
  const totalCongelada = congeladaInvs.reduce(
    (sum, inv) => sum + calcMetrics(inv).valorActual, 0
  );

  // How many investments are stale (>= 30 days without valuation update)
  const staleCount = investments.filter((inv) => {
    if (inv.valuaciones.length === 0) return true;
    const sorted = [...inv.valuaciones].sort(
      (a, b) => b.fecha.toMillis() - a.fecha.toMillis()
    );
    const days = Math.floor(
      (Date.now() - sorted[0].fecha.toMillis()) / (1000 * 60 * 60 * 24)
    );
    return days >= 30;
  }).length;
      ...
      {investments.length > 0 && (
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div className="bg-[#161F2C] border border-white/5 rounded-xl p-3">
            ...
            <p className="text-base font-semibold text-[#22D3EE] tabular-nums">
              {fmt(totalVista)}
            </p>
          ...
          <div className="bg-[#161F2C] border border-white/5 rounded-xl p-3">
            ...
            <p className="text-base font-semibold text-[#64748B] tabular-nums">
              {fmt(totalCongelada)}
            </p>
            <p className="text-xs text-[#8899AA]">
          ...
          {vistaInvs.length > 0 && (
            <div>
              ...
              <div className="space-y-3">
                {vistaInvs.map((inv) => (
                  <InvestmentCard key={inv.id} investment={inv} onEdit={openEdit} />
                ))}
              </div>
            </div>
          )}
